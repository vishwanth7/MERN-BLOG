    ...
    const handleSave=async()=>{
      try{
        ...
        const res=await fetch(`http://localhost:3000/api/comment/editComment/${comment._id}`,{
            ...
            body:JSON.stringify({content:editedContent,token})
        })
        // const data=await res.json()
        if(res.ok){
            setIsEditing(false)
            ...
            <div className='flex items-center pt-2 text-sm border-t dark:border-gray-700 max-w-fit gap-2'>
                    ...
                    }</p>
                {
                    ...
                    currentUser && (currentUser._id===comment.userId || currentUser.isAdmin) &&(
                        <>
                        <button type='button' className='text-gray-400 hover:text-red-400' onClick={handleEdit}>
                            Edit
                        </button>
                        <button type='button' className='text-gray-400 hover:text-red-400' onClick={()=>onDelete(comment._id)}>
                        Delete
                        </button>
                        </>
